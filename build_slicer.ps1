...
if (!$allPrereqsOk) {
    ...
    Write-Host "1. CMake >= 3.20.6 and less than 4.0" -ForegroundColor Red
    Write-Host "2. Git >= 1.7.10" -ForegroundColor Red
...
# Build Slicer
Write-Host ""
Write-Host "Building Slicer ($BuildType mode)..." -ForegroundColor Cyan
Write-Host "This will take several hours (3-4 hours on desktop, 8-12 hours on laptop)" -ForegroundColor Yellow
Write-Host "Build requires:"
if ($BuildType -eq "Release") {
    Write-Host "- Disk space: ~15GB" -ForegroundColor Yellow
} else {
    Write-Host "- Disk space: ~60GB" -ForegroundColor Yellow
}
